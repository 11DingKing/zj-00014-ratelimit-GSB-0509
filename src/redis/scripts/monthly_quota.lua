...
-- 计算当前日期后缀(YYYY-MM)
local current_date = os.date("!%Y-%m", current_ms / 1000)

-- 生成当前月的 key
local monthly_key = key_prefix .. ":" .. current_date

-- 获取当前月已使用配额
local used_quota = redis.call('GET', monthly_key) or 0
used_quota = tonumber(used_quota)

-- 检查是否超过配额
if used_quota + weight > monthly_limit then
    -- 超过配额，拒绝请求
    local remaining = monthly_limit - used_quota
    ...
    -- 构造下月1号的时间戳
    local next_month_start = os.time({year = next_year, month = next_month, day = 1, hour = 0, min = 0, sec = 0})
    local reset_at = next_month_start * 1000
    
    return {0, remaining, reset_at}
else
    -- 在配额内，扣减配额
    local new_used = redis.call('INCRBY', monthly_key, weight)
    
    -- 设置过期时间(2个月，防止过期)
    redis.call('EXPIRE', monthly_key, 60 * 24 * 30 * 2)
    
    local remaining = monthly_limit - new_used
