...
export class PolicyManager {
  ...
  private getWhitelistCacheKey(resource: string, subject: string): string {
    return `whitelist:${resource}:${subject}`;
  }

  async isWhitelisted(resource: string, subject: string): Promise<boolean> {
    const redis = getRedisClient();
    const cacheKey = this.getWhitelistCacheKey(resource, subject);
